...
void printDateUserAndSeed(int seed, std::ofstream& outfile) {
...
void MC_summary(int seed, const std::string& output_filename) {

    // Apri il file di output in modalità scrittura
    std::ofstream outfile(output_filename, std::ios::app);

    if (!outfile) {
        std::cerr << "Impossibile aprire il file di output '" << output_filename << "'" << std::endl;
        return;
    }

    // Chiama la funzione per stampare il nome utente e il seed su un file
    printDateUserAndSeed(seed, outfile);

    // Apri il file "run.mac" in modalità lettura
    std::ifstream run_file("run.mac");
    if (!run_file) {
    ...
    while (std::getline(run_file, line)) {
        ...
        } else if (line.find("/my_gun/spread/radiusSpread") != std::string::npos) {
    ...
    while (std::getline(construction_file, line)) {
        ...
        } else if (line.find("/my_construction/MaterialOfLightGuide") != std::string::npos) {
